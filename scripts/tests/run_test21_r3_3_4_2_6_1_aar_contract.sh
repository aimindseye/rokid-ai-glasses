...
printf '%s\n' \
  '============================================================' \
  'TEST 21 r3.3.4.2.6.1 — EXACT CXR-L AAR BINDER CONTRACT' \
...
PYTHONDONTWRITEBYTECODE=1 python3 "$REPO/scripts/tests/check_test21_r3_3_4_2_6_1_source_contract.py" --repo "$REPO"
CHECK_RC=$?
if [ "$CHECK_RC" -ne 0 ]; then
  echo "TEST21_R3_3_4_2_6_1_SOURCE_CONTRACT=FAIL"
...
ANALYZER="$REPO/scripts/tests/analyze_test21_r3_3_4_2_6_1_aar_contract.py"
...
if [ "$ANALYZE_RC" -eq 0 ]; then
  PYTHONDONTWRITEBYTECODE=1 python3 "$REPO/scripts/tests/package_test21_r3_3_4_2_6_1_sanitized.py" --evidence "$OUTPUT"
  PACKAGE_RC=$?
else
  PACKAGE_RC=99
fi
