#!/usr/bin/env bash
REPO="";PHONE="";OUTPUT="";PROFILE="EXACT_CXRL_LOCAL_ACTIVATION"
...
DURATION=18;POLL=.10;SNAP=.75
if [ "${TEST21_R3_3_4_TEST_MODE:-0}" = 1 ];then DURATION=3;POLL=.05;SNAP=.4;fi
...
[ "$PROFILE" = EXACT_CXRL_LOCAL_ACTIVATION ]||{ echo "ERROR: unsupported profile";exit 2; }
...
CHECK="$REPO/scripts/tests/check_test21_r3_3_4_source_contract.py";COLLECT="$REPO/scripts/tests/collect_test21_r3_3_4_local_activation.py";ANALYZE="$REPO/scripts/tests/analyze_test21_r3_3_4_local_activation.py"
mkdir -p "$OUTPUT/raw" "$OUTPUT/sanitized"||exit 1
TTY_IN=/dev/tty;TTY_OUT=/dev/tty;if [ "${TEST21_R3_3_4_TEST_MODE:-0}" = 1 ]||[ ! -r "$TTY_IN" ];then TTY_IN=/dev/stdin;TTY_OUT=/dev/stdout;fi
...
logmark(){ "$ADB" -s "$PHONE" shell log -t Test21R334 "$RUN_ID|$1" >/dev/null 2>&1||true;}
snapshot(){ stem="$1";"$ADB" -s "$PHONE" shell dumpsys activity services "$HI_ROKID" >"$OUTPUT/raw/$stem-hi-services-private.txt" 2>&1;"$ADB" -s "$PHONE" shell dumpsys activity providers "$HI_ROKID" >"$OUTPUT/raw/$stem-hi-providers-private.txt" 2>&1;}
echo "============================================================";echo "TEST 21 r3.3.4 — EXACT CXR-L LOCAL COMPONENT ACTIVATION";echo "============================================================";echo "PROFILE=$PROFILE";echo "PCAPDROID_OPERATION=NONE";echo "NETWORK_CAPTURE=NONE";echo "PHOTO_OPERATION=NONE";echo "AUDIO_OPERATION=NONE"
...
if ! prompt HI_ROKID_CONNECTED "[PHONE CHECK ONLY] Confirm Hi Rokid is open and the glasses are normally connected. Do not use camera, Assistant, translation, recording, or any media feature.";then echo "ERROR: baseline not confirmed";exit 1;fi
"$ADB" -s "$PHONE" shell dumpsys package "$HI_ROKID" >"$OUTPUT/raw/hi-package-dumpsys-private.txt" 2>&1;snapshot baseline
RUN_ID="test21-r3-3-4-$(date -u +%Y%m%dT%H%M%SZ)";TOKEN="$(python3 -c 'import secrets;print(secrets.token_hex(32))')"
"$ADB" -s "$PHONE" shell am force-stop "$CUSTOM_PACKAGE" >/dev/null 2>&1;"$ADB" -s "$PHONE" shell am start -W --user "$ACTIVE_USER" -n "$CUSTOM_LAUNCHER" --es run_id "$RUN_ID" --es firmware_label test21-r3-3-4-local-activation --es operator_gate_token "$TOKEN" >"$OUTPUT/raw/custom-launch-private.txt" 2>&1||exit 1
...
"$ADB" -s "$PHONE" logcat -T 1 -b events -v epoch >"$OUTPUT/raw/activity-events-private.txt" 2>&1 & P1=$!;"$ADB" -s "$PHONE" logcat -T 1 -b system -b main -v epoch ActivityManager:V ActivityTaskManager:V ContentProviderHelper:V Test21R334:I '*:S' >"$OUTPUT/raw/activity-manager-private.txt" 2>&1 & P2=$!
...
if ! prompt READY_FOR_R3_3_4_CONNECTION "[TERMINAL ACTION ONLY] Keep the custom app visible. The host will now start local Android component observers. Do NOT tap button 2 until the NOW instruction.";then echo "ERROR: readiness not confirmed";exit 1;fi
READY_FILE="$OUTPUT/raw/collector-ready.txt";rm -f "$READY_FILE";python3 "$COLLECT" --adb "$ADB" --phone "$PHONE" --hi-package "$HI_ROKID" --custom-package "$CUSTOM_PACKAGE" --remote-events "$REMOTE_EVENTS" --output "$OUTPUT/raw" --duration-seconds "$DURATION" --poll-seconds "$POLL" --snapshot-seconds "$SNAP" --ready-file "$READY_FILE" >"$OUTPUT/raw/collector-console-private.txt" 2>&1 & COLLECTOR_PID=$!;j=1;while [ "$j" -le 50 ]&&[ ! -s "$READY_FILE" ];do sleep .1;j=$((j+1));done;[ -s "$READY_FILE" ]||{ echo "ERROR: collector readiness timeout";exit 1; };grep -q "^HI_PROCESS_VISIBLE=NO$" "$READY_FILE"||{ echo "ERROR: collector started with Hi Rokid already visible";exit 1; };echo "COLLECTOR_READY_PRECONNECTION=PASS";logmark BUTTON2_NOW_PROMPT
...
EOF
python3 "$ANALYZE" --evidence "$OUTPUT";ARC=$?;echo "TEST21_R3_3_4_RUN_RC=$ARC";echo "PRIVATE_EVIDENCE_ROOT=$OUTPUT";echo "HI_ROKID_RESTORATION=$RECOVERY";echo "PCAPDROID_OPERATION=NONE";echo "NETWORK_CAPTURE=NONE";echo "PHOTO_OPERATION=NONE";echo "AUDIO_OPERATION=NONE";exit "$ARC"
