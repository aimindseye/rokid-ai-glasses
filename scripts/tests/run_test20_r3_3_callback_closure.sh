...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/tests/run_test20_r3_3_callback_closure.sh \
    --repo PATH \
    --phone ADB_SERIAL \
    --firmware EXACT_VERSION \
    --firmware-screenshot IMAGE \
    --output DIR \
    [--profile STRONG_REF_PRECONNECT|POSTCONNECT_REREGISTER|ARG3_ZERO_DIAGNOSTIC] \
    [--expected-hi-rokid-version VERSION] \
    [--remote-events DEVICE_PATH]

r3.3 preserves the accepted two-phase one-shot gate and instruments the
post-takePhoto callback path. Run exactly one diagnostic profile per fresh run.
Never advance to another profile unless the final analyzer explicitly names it.
EOF
}
...
case "$PROFILE" in
  STRONG_REF_PRECONNECT|POSTCONNECT_REREGISTER|ARG3_ZERO_DIAGNOSTIC) ;;
...
RUN_ID="cxrl-photo-callback-r3.3-${PROFILE}-${FIRMWARE}-$(date -u +%Y%m%dT%H%M%SZ)"
...
SOURCE_CHECK="$REPO/scripts/tests/check_test20_r3_3_source_contract.py"
SDK_INSPECTOR="$REPO/scripts/tests/inspect_test20_r3_3_sdk_contract.sh"
ANALYZER="$REPO/scripts/tests/analyze_test20_r3_3_callback_closure.py"
[ -x "$SOURCE_CHECK" ] || die "source-contract checker missing: $SOURCE_CHECK"
[ -x "$SDK_INSPECTOR" ] || die "r3.3 SDK inspector missing: $SDK_INSPECTOR"
[ -x "$ANALYZER" ] || die "r3.3 analyzer missing: $ANALYZER"

python3 "$SOURCE_CHECK" --repo "$REPO" --output "$OUTPUT/diagnostics/source-contract.json"
SOURCE_RC=$?
[ "$SOURCE_RC" -eq 0 ] || die "r3.3 source contract is not safe enough to run"

"$SDK_INSPECTOR" --repo "$REPO" --output "$OUTPUT/diagnostics/sdk-contract"
SDK_RC=$?
[ "$SDK_RC" -eq 0 ] || die "r3.3 SDK contract inspection failed; no media action was performed"
...
APP_DUMP_RC=$?
[ "$APP_DUMP_RC" -eq 0 ] || die "r3.3 callback-closure app is not installed"
...
[ "$APP_VERSION" = "$BASE_VERSION" ] ||
  die "r3.3 app version mismatch: expected $BASE_VERSION, found ${APP_VERSION:-UNRESOLVED}"
...
TEST20_R3_3_SCHEMA=rokid.test20-r3.3.run-metadata.v1
RUN_ID=$RUN_ID
FIRMWARE=$FIRMWARE
PHONE_SERIAL_SHA256=$PHONE_HASH
HI_ROKID_VERSION=$HI_VERSION
PACKAGE=$PKG
APP_VERSION_NAME=$BASE_VERSION
CALLBACK_PROFILE=$PROFILE
PROFILE_ADVANCE_POLICY=ANALYZER_EXPLICIT_ONLY
...
PHOTO_ARG_1=1920
PHOTO_ARG_2=1080
PHOTO_ARG_3=$( [ "$PROFILE" = "ARG3_ZERO_DIAGNOSTIC" ] && echo 0 || echo 80 )
PHOTO_ARGUMENT_SEMANTICS=$( [ "$PROFILE" = "ARG3_ZERO_DIAGNOSTIC" ] && echo DIAGNOSTIC_THIRD_ARGUMENT_ZERO_SEMANTICS_NOT_ASSUMED || echo WORKING_HYPOTHESIS_WIDTH_HEIGHT_QUALITY_NOT_GENERALIZED )
POST_TAKEPHOTO_WATCHDOG_MS=1000,5000,10000,20000,29000
...
case "$LAUNCHER" in
  "$PKG"/*) ;;
  *) die "could not resolve r3.3 test app launcher" ;;
esac
...
[ -n "$ACTUAL_REMOTE" ] || die "could not locate r3.3 JSONL event stream; photo remains unarmed"
...
"$ADB" -s "$PHONE" logcat -d -v epoch > "$OUTPUT/private/logcat-private.txt" 2>&1
LOGCAT_RC=$?
[ "$LOGCAT_RC" -eq 0 ] || printf 'WARN: private logcat capture failed\n' >&2

if prompt_exact "NO_OTHER_MEDIA_ACTION" "Attest only to the concrete operator action: outside the single r3.3 photo action, no other camera, photo, audio, translation, Assistant, or media action was performed during the armed phase."; then
...
TEST20_R3_3_SCHEMA=rokid.test20-r3.3.operator-attestation.v1
CALLBACK_PROFILE=$PROFILE
...
python3 "$ANALYZER" \
  ...
  --profile "$PROFILE" \
  --summary "$OUTPUT/sanitized/test20-r3-3-callback-closure-summary.json" \
...
grep -E '^TEST20_R3_3_|^AUDIO_OPERATION=' "$OUTPUT/final-gate.txt" > "$OUTPUT/sanitized/final-gate-status.txt" 2>/dev/null || true
