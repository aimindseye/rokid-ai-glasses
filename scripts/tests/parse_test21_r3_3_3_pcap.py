...
# TEST21_R3_3_3_PCAP_COMPATIBILITY_SHIM=YES
...
def trailer(data):
    return _core.trailer(data, 80)
def parse_ip(frame, end):
    return _core.parse_ip(frame, end, 1, "legacy_ethernet_fallback")
def parse_pcap(path, uidmap):
    return _core.parse_pcap(Path(path), uidmap, _core.get_profile(REVISION))
def tshark_http(pcap, keylog, flowmap):
    rows=[{"frame_number":None,"flow_key":k,"package":v} for k,v in flowmap.items()]
