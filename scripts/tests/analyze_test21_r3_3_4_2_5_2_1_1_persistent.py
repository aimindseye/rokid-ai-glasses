...
def main():
    ...
    private = {
        'schema': 'rokid.test21-r3.3.4.2.5.2.1.1.private.v1',
        'collection': meta,
        'dex_evidence': evidence,
        'stub_subclasses': subclasses,
        'service_onbind': onbind,
    }
    (priv / 'r3-3-4-2-5-2-1-1-private.json').write_text(json.dumps(private, indent=2, sort_keys=True) + '\n')
    ...
    summary = {
        'schema': 'rokid.test21-r3.3.4.2.5.2.1.1.sanitized.v1',
        'analysis': 'PASS',
        ...
        'proof_boundary': 'ONE_PERSISTENT_ROOT_SHELL_FRAMED_BOUNDED_PROC_MEM_READS_EXACT_COVERAGE_ACCOUNTING_PARSED_DEX_CLASS_DEF_NO_FRIDA_NO_PTRACE_NO_SIGNAL_NO_PAYLOAD_EXECUTION',
    }

    (san / 'test21-r3-3-4-2-5-2-1-1-summary.json').write_text(json.dumps(summary, indent=2, sort_keys=True) + '\n')
    ...
    keys = [
        ('TEST21_R3_3_4_2_5_2_1_1_ANALYSIS', 'PASS'),
        ('ROOT_PROBE', 'AVAILABLE'),
    ...
    (san / 'test21-r3-3-4-2-5-2-1-1-summary.txt').write_text('\n'.join(lines) + '\n')
    print('\n'.join(lines))
    return 0
