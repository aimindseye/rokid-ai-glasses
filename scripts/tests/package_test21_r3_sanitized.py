...
def main():
    ...
    a = ap.parse_args()
    rc, out, side = package(ROOT, 'r3', Path(a.evidence), a.phone, Path(a.output) if getattr(a, 'output', '') else None, quiet=True)
    if rc != 0:
        return rc
    print('TEST21_R3_SANITIZED_PACKAGE=PASS')
    ...
    print('AUTHORIZATION_TOKEN_VALUE_INCLUDED=NO')
    print('PROCESS_IDENTIFIER_INCLUDED=NO')
    print('PHOTO_PAYLOAD_INCLUDED=NO')
    print('AUDIO_PAYLOAD_INCLUDED=NO')
    return 0
