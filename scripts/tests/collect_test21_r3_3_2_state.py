...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--adb',required=True);ap.add_argument('--phone',required=True);ap.add_argument('--hi-package',required=True);ap.add_argument('--custom-package',required=True);ap.add_argument('--output',required=True);ap.add_argument('--label',required=True);a=ap.parse_args()
 out=Path(a.output);out.mkdir(parents=True,exist_ok=True);label=re.sub(r'[^A-Za-z0-9_.-]+','-',a.label)
 ...
 resumed='\n'.join(x for x in acts.splitlines() if 'ResumedActivity' in x or 'topResumedActivity' in x)
 fg='YES' if a.custom_package in resumed else 'NO'
