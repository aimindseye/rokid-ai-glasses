...
def main():
    ...
    a = ap.parse_args()
    rc, out, side = package(ROOT, 'r3.3.4.1', Path(a.evidence), a.phone, None, quiet=True)
    if rc != 0:
        return rc
    print('TEST21_R3_3_4_1_SANITIZED_PACKAGE=PASS')
    print('SANITIZED_ZIP=' + str(out))
    print('SANITIZED_ZIP_SHA256=' + hashlib.sha256(out.read_bytes()).hexdigest())
    print('RAW_LOGCAT_INCLUDED=NO')
    print('RAW_DUMPSYS_INCLUDED=NO')
    print('APK_INCLUDED=NO')
