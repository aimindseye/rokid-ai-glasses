...
def main():
 ...
 while time.monotonic()-start<a.duration_seconds:
  ms=int((time.monotonic()-start)*1000); pids=sh(a.adb,a.phone,['pidof',a.hi_package])[1].strip(); pst=ps(a.adb,a.phone); rows.append({'elapsed_ms':ms,'hi_visible':bool(pids)})
  if pids and first is None: first=ms; (out/'first-respawn-ps-private.txt').write_text(pst); snap(a.adb,a.phone,a.hi_package,a.custom_package,out)
