...
BASE="/data/local/tmp/rokid-test21-r32521111-$$"
...
while IFS='|' read -r op req a b c; do
  ...
  case "$op" in
    ...
    PING)
      printf 'PONG\n' > "$out"; rc=0; emit_file "$req" "$rc" "$out" "$err" ;;
    MAPS)
...
def build_remote_worker_command(worker_script: str = WORKER_SCRIPT) -> str:
    # adb shell -T takes a single remote command string.  Quote the complete
    # worker as the one argument to Magisk su -c so the Android shell cannot
    # split `-c` away from its command.  This exact transport was qualified on
    # the rooted Pixel 7 with WORKER_READY / uid=0 / PONG / BYE.
    return 'su -c ' + shlex.quote(worker_script)
...
class RootWorker:
    def __init__(self, adb: str, phone: str, startup_timeout: float = 12, worker_script: str = WORKER_SCRIPT):
        remote_command = build_remote_worker_command(worker_script)
        self.proc = subprocess.Popen(
            [adb, '-s', phone, 'shell', '-T', remote_command],
        ...
        if ready != 'WORKER_READY':
            ...
            try:
                if self.proc.stderr is not None:
                    ready_fd, _, _ = select.select([self.proc.stderr.fileno()], [], [], 0)
                    if ready_fd:
                        extra = os.read(self.proc.stderr.fileno(), 4096).decode('utf-8', 'replace').replace('\r', '').strip()
                        if extra:
                            diag = diag + ' stderr=' + extra[:512]
            except Exception:
                pass
            self.close(force=True)
            raise RuntimeError('persistent root worker did not become ready: ' + diag)
...
def main():
    ...
    try:
        ...
        frame = worker.request('PING', [], min(8, max(1, global_deadline - time.monotonic())))
        pong = frame['payload'].decode('utf-8', 'replace').replace('\r', '').strip()
        if frame['rc'] != 0 or pong != 'PONG':
            print('PERSISTENT_ROOT_SESSION_QUALIFICATION=FAIL')
            print('ERROR: persistent root protocol PING/PONG failed', file=sys.stderr)
            return 6
        print('PERSISTENT_ROOT_SESSION_QUALIFICATION=PASS')
        print('PERSISTENT_ROOT_TRANSPORT=ADB_SHELL_T')
        print('MAGISK_SU_COMMAND_QUOTING=PASS')
        ...
        private = {
            'schema': 'rokid.test21-r3.3.4.2.5.2.1.1.1.persistent-root.private.v1',
            'root_id': root_id,
            'pid': pid,
            'process_maps_access': 'READABLE',
            'external_proc_mem_access': external_access,
            'persistent_root_session': True,
            'persistent_root_transport': 'ADB_SHELL_T',
            'persistent_root_protocol_qualification': 'PASS',
            'magisk_su_command_quoting': 'PASS',
