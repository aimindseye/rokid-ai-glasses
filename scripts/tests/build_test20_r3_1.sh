...
usage(){ echo "Usage: bash scripts/tests/build_test20_r3_1.sh --repo <repo> --sdk-version 1.0.1 --output <dir>"; }
...
echo "Test 20 r3.1 governed build-only stage"; echo "======================================="
...
( cd "$REPO/android-client" || exit 90; ./gradlew --no-daemon :test20r31:assembleDebug -ProkidCxrLVersion="$SDK_VERSION" ) >"$OUTPUT/gradle-build.log" 2>&1
RC=$?; echo "GRADLE_BUILD_EXIT_CODE=$RC"; [ "$RC" -eq 0 ] || { tail -n 160 "$OUTPUT/gradle-build.log"; exit 1; }
SRC="$REPO/android-client/test20r31/build/outputs/apk/debug/test20r31-debug.apk"; APK="$OUTPUT/apk/test20-r3-1-debug.apk"; [ -s "$SRC" ] || { fail "APK missing"; exit 1; }; cp "$SRC" "$APK" || exit 1
...
for x in "package: name='org.aimindseye.rokid.cxrmediapreflight'" "versionCode='1'" "versionName='1.0-test20-r3.1'"; do grep -Fq "$x" "$OUTPUT/apk-badging.txt" || { fail "APK identity mismatch: $x"; exit 1; }; done
...
if [ -d "$CACHE" ]; then for f in $(find "$CACHE" -type f -name '*.aar' -print 2>/dev/null); do [ "$(shasum -a 256 "$f"|awk '{print $1}')" = "$EXPECTED_AAR" ] && FOUND_AAR=YES; done; for f in $(find "$CACHE" -type f -name '*.pom' -print 2>/dev/null); do [ "$(shasum -a 256 "$f"|awk '{print $1}')" = "$EXPECTED_POM" ] && FOUND_POM=YES; done; fi
...
TEST20_R3_1_SCHEMA=rokid.test20-r3.1.build-attestation.v1
...
echo "TEST20_R3_1_EXACT_ARTIFACT_IDENTITY=PASS"; echo "TEST20_R3_1_APK_BUILD=PASS"; echo "TEST20_R3_1_APK_IDENTITY=PASS"; echo "TEST20_R3_1_MERGED_APK_PERMISSION_GATE=PASS"; echo "TEST20_R3_1_BUILD_EVIDENCE=PASS"; echo "TEST20_R3_1_APK_SHA256=$APK_SHA"; echo "TEST20_R3_1_BUILD_OUTPUT=$OUTPUT"; echo "PHONE_OPERATION=NONE"; echo "GLASSES_OPERATION=NONE"; echo "TEST20_R3_1_READY_FOR_INSTALL_STAGE=YES"
