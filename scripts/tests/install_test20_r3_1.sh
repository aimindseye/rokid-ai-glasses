...
usage(){ echo "Usage: bash scripts/tests/install_test20_r3_1.sh --repo <repo> --phone <serial> --evidence-dir <dir> --expected-hi-rokid-version G1.11.11.0727 [--reset-app-data]"; }
...
echo "Test 20 r3.1 governed install stage"; echo "====================================="
[ -n "$REPO" ] && [ -n "$PHONE" ] && [ -n "$EVIDENCE_DIR" ] && [ -n "$EXPECTED_HI_ROKID_VERSION" ] || { usage >&2; exit 2; }
APK="$EVIDENCE_DIR/apk/test20-r3-1-debug.apk"; ATT="$EVIDENCE_DIR/build-attestation.txt"; [ -s "$APK" ] && [ -s "$ATT" ] || { fail "build evidence incomplete"; exit 1; }
...
PACKAGE=org.aimindseye.rokid.cxrmediapreflight; APP="$("$ADB" -s "$PHONE" shell dumpsys package "$PACKAGE" 2>/dev/null)"; VN="$(printf '%s\n' "$APP"|sed -n 's/.*versionName=//p'|head -1|tr -d '\r')"; VC="$(printf '%s\n' "$APP"|sed -n 's/.*versionCode=\([0-9][0-9]*\).*/\1/p'|head -1)"; [ "$VN" = 1.0-test20-r3.1 ] && [ "$VC" = 1 ] || { fail "installed identity mismatch"; exit 1; }
...
echo "TEST20_R3_1_INSTALLED_PACKAGE_IDENTITY=PASS"; echo "TEST20_R3_1_HI_ROKID_BASELINE=PASS"; echo "TEST20_R3_1_INSTALL=PASS"; echo "TEST20_R3_1_INSTALL_EVIDENCE=$OUT"; echo "HI_ROKID_DATA_MUTATION=NONE"; echo "BLUETOOTH_PAIRING_MUTATION=NONE"; echo "REBOOT_OPERATION=NONE"; echo "MEDIA_OPERATION=NONE"
