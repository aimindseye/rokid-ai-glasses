#!/usr/bin/env bash
REPO="";PHONE="";OUTPUT="";PROFILE="AUTHORIZED_FOREGROUND_TO_BACKGROUND_HOME_15S"
HI_ROKID="com.rokid.sprite.global.aiapp";CUSTOM_PACKAGE="org.aimindseye.rokid.cxrphotoqualification";EXPECTED_CUSTOM_VERSION="1.0-test20-final";BACKGROUND_SECONDS=15;OBSERVATION_SECONDS=30
if [ "${TEST21_R3_3_2_TEST_MODE:-0}" = "1" ];then BACKGROUND_SECONDS=2;OBSERVATION_SECONDS=3;fi
...
[ "$PROFILE" = "AUTHORIZED_FOREGROUND_TO_BACKGROUND_HOME_15S" ]||{ echo "ERROR: only AUTHORIZED_FOREGROUND_TO_BACKGROUND_HOME_15S is enabled";exit 2; }
...
CHECK="$REPO/scripts/tests/check_test21_r3_3_2_source_contract.py";STATE="$REPO/scripts/tests/collect_test21_r3_3_2_state.py";RESP="$REPO/scripts/tests/collect_test21_r3_3_2_respawn.py";ANALYZER="$REPO/scripts/tests/analyze_test21_r3_3_2_lifecycle_transition.py"
mkdir -p "$OUTPUT/raw" "$OUTPUT/sanitized"||exit 1
TTY_IN=/dev/tty;TTY_OUT=/dev/tty;if [ "${TEST21_R3_3_2_TEST_MODE:-0}" = 1 ]||[ ! -r "$TTY_IN" ];then TTY_IN=/dev/stdin;TTY_OUT=/dev/stdout;fi
...
echo "============================================================";echo "TEST 21 r3.3.2 — AUTHORIZED FOREGROUND→BACKGROUND LIFECYCLE TRANSITION";echo "============================================================";echo "PROFILE=AUTHORIZED_FOREGROUND_TO_BACKGROUND_HOME_15S";echo "LIFECYCLE_TRANSITION_METHOD=HOST_KEYCODE_HOME";echo "CXR_L_CONNECTION_ATTEMPT=NONE";echo "SECONDARY_PACKAGE_FORCE_STOP=NONE";echo "PHOTO_OPERATION=NONE";echo "AUDIO_OPERATION=NONE"
...
RUN_ID="test21-r3-3-2-$(date -u +%Y%m%dT%H%M%SZ)";TOKEN="$(python3 -c 'import secrets; print(secrets.token_hex(32))')"
"$ADB" -s "$PHONE" shell am force-stop "$CUSTOM_PACKAGE" >/dev/null 2>&1||exit 1;"$ADB" -s "$PHONE" shell am start -W --user "$ACTIVE_USER" -n "$CUSTOM_LAUNCHER" --es run_id "$RUN_ID" --es firmware_label "test21-r3-3-2-lifecycle" --es operator_gate_token "$TOKEN" >"$OUTPUT/raw/custom-launch-private.txt" 2>&1||exit 1
echo;echo "=== AUTHORIZATION ===";echo "[PHONE ACTION] Tap ONLY '1. Authorize through Hi Rokid' once. Complete authorization, return to the custom app, and leave the custom app visible. DO NOT tap any other control."
if ! prompt AUTHORIZED_BACKGROUND_TRANSITION_READY "[PHONE ACTION BEFORE TYPING] After authorization, return to the custom app and leave it visible. The host will perform the Home transition after this confirmation. DO NOT press Home yourself.";then echo "ERROR: lifecycle profile not confirmed";exit 1;fi
...
python3 "$STATE" --adb "$ADB" --phone "$PHONE" --hi-package "$HI_ROKID" --custom-package "$CUSTOM_PACKAGE" --output "$OUTPUT/raw" --label foreground-before||exit 1
grep -q '^CUSTOM_PROCESS_VISIBLE=YES$' "$OUTPUT/raw/foreground-before-state.txt"&&grep -q '^CUSTOM_FOREGROUND_PROVEN=YES$' "$OUTPUT/raw/foreground-before-state.txt"||{ echo "ERROR: custom app foreground baseline not proven";exit 1; }
"$ADB" -s "$PHONE" logcat -T 1 -b events -v epoch >"$OUTPUT/raw/lifecycle-activity-events-private.txt" 2>&1 & P1=$!;"$ADB" -s "$PHONE" logcat -T 1 -v epoch ActivityManager:I ActivityTaskManager:I '*:S' >"$OUTPUT/raw/lifecycle-activity-task-private.txt" 2>&1 & P2=$!
echo;echo "=== CONTROLLED FOREGROUND → BACKGROUND TRANSITION ===";echo "[PHONE ACTION] NONE. The host will press Android Home once through ADB."
"$ADB" -s "$PHONE" shell input keyevent KEYCODE_HOME >"$OUTPUT/raw/home-transition-private.txt" 2>&1||{ echo "ERROR: host Home transition failed";exit 1; }
sleep 1;python3 "$STATE" --adb "$ADB" --phone "$PHONE" --hi-package "$HI_ROKID" --custom-package "$CUSTOM_PACKAGE" --output "$OUTPUT/raw" --label background-00||exit 1
grep -q '^CUSTOM_PROCESS_VISIBLE=YES$' "$OUTPUT/raw/background-00-state.txt"&&grep -q '^CUSTOM_FOREGROUND_PROVEN=NO$' "$OUTPUT/raw/background-00-state.txt"||{ echo "ERROR: custom app background transition not proven immediately after Home";exit 1; }
echo;echo "=== BACKGROUND SETTLE ===";echo "[PHONE ACTION] NONE. Leave the phone untouched while the custom process remains alive in background."
sleep "$BACKGROUND_SECONDS";python3 "$STATE" --adb "$ADB" --phone "$PHONE" --hi-package "$HI_ROKID" --custom-package "$CUSTOM_PACKAGE" --output "$OUTPUT/raw" --label background-15||exit 1
grep -q '^CUSTOM_PROCESS_VISIBLE=YES$' "$OUTPUT/raw/background-15-state.txt"&&grep -q '^CUSTOM_FOREGROUND_PROVEN=NO$' "$OUTPUT/raw/background-15-state.txt"||{ echo "ERROR: custom process/background state not preserved";exit 1; }
stopobs;pull_events "$OUTPUT/raw/pre-force-events-private.jsonl"||exit 1;verify_events "$OUTPUT/raw/pre-force-events-private.jsonl"||exit 1
"$ADB" -s "$PHONE" logcat -T 1 -b events -v epoch >"$OUTPUT/raw/postforce-activity-events-private.txt" 2>&1 & P1=$!;"$ADB" -s "$PHONE" logcat -T 1 -v epoch ActivityManager:I ActivityTaskManager:I '*:S' >"$OUTPUT/raw/postforce-activity-manager-private.txt" 2>&1 & P2=$!
echo;echo "=== CONTROLLED HI ROKID FORCE-STOP ===";echo "[PHONE ACTION] NONE.";"$ADB" -s "$PHONE" shell am force-stop "$HI_ROKID" >"$OUTPUT/raw/hi-force-stop-private.txt" 2>&1;rc=$?;[ "$rc" -eq 0 ]||exit 1;HI_FORCE_STOP_ISSUED=1
abs=NO;i=1;while [ "$i" -le 10 ];do p="$($ADB -s "$PHONE" shell pidof "$HI_ROKID" 2>/dev/null|tr -d '\r')";[ -z "$p" ]&&{ abs=YES;break; };sleep 1;i=$((i+1));done
"$ADB" -s "$PHONE" shell ps -A -o USER,PID,PPID,NAME,ARGS >"$OUTPUT/raw/ps-immediate-post-force-private.txt" 2>&1||"$ADB" -s "$PHONE" shell ps -A >"$OUTPUT/raw/ps-immediate-post-force-private.txt" 2>&1;same="$(grep -F "$HI_ROKID" "$OUTPUT/raw/ps-immediate-post-force-private.txt" 2>/dev/null|wc -l|tr -d ' ')";printf 'HI_PROCESS_ABSENT_OBSERVED=%s\nSAME_PACKAGE_PROCESS_COUNT_POST_FORCE=%s\n' "$abs" "$same" >"$OUTPUT/raw/force-stop-observation.txt";[ "$abs" = YES ]&&[ "$same" -eq 0 ]||{ echo "ERROR: full Hi Rokid package stop not proven";exit 1; }
...
trap - EXIT;echo "TEST21_R3_3_2_RUN=PASS";echo "PRIVATE_EVIDENCE_ROOT=$OUTPUT";echo "PHOTO_OPERATION=NONE";echo "AUDIO_OPERATION=NONE";exit 0
