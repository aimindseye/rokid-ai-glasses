...
paths = [Path(p.decode()) for p in list_file.read_bytes().split(b"\0") if p]
blocked_suffixes = {
    ".pcap", ".pcapng", ".sslkeylog", ".har", ".apk", ".apks",
    ".bugreport", ".hci", ".btsnoop"
}
patterns = {
    "macOS absolute private path": re.compile(b"/" + b"Users" + rb"/[A-Za-z0-9._-]+/"),
    "private test root": re.compile(b"rokid" + b"-nettest"),
    "TLS traffic secret": re.compile(rb"(?:CLIENT|SERVER)_" + rb"(?:HANDSHAKE_)?" + b"TRAFFIC_SECRET"),
    "TLS exporter secret": re.compile(b"EXPORTER" + b"_SECRET"),
    "AWS-style secret label": re.compile(b"AWS_SECRET" + b"_ACCESS_KEY"),
    "private key block": re.compile(b"-----BEGIN " + rb"(?:RSA |EC |OPENSSH )?" + b"PRIVATE KEY-----"),
}
problems = []
for path in paths:
