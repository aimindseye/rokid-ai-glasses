#!/usr/bin/env python3
"""Controlled stock Hi Rokid ADB-toggle capture for r1.3.3.2.25.3.1.
...
class Device:
    ...
    def dump_ui(self, local: Path) -> str:
        remote = f"/sdcard/r25_3_1_ui_{os.getpid()}.xml"
...
def operator_ready(prompt: str, test_auto: bool) -> None:
    print(prompt, flush=True)
    if test_auto:
        return
    try:
        input("Press Enter only when ready to arm this stock action: ")
    except EOFError as exc:
        raise CaptureFailure("operator input ended before action") from exc
...
def print_plan(initial: str, cycles: int) -> int:
    ...
    print(f"R25_3_1_PLAN_RELEASE={RELEASE}")
    print("R25_3_1_PLAN_DEVICE_CONTACT=NO")
    print("R25_3_1_DISABLE_ORACLE=PERSIST_VENDOR_ADB_FALSE_AND_UI_SWITCH_OFF")
    print("R25_3_1_ENABLE_ORACLE=PERSIST_VENDOR_ADB_TRUE_AND_UI_SWITCH_ON")
    print("R25_3_1_ADB_TRANSPORT_DISAPPEARANCE_REQUIRED=NO")
    print("R25_3_1_CONTROL_CHANNEL_USABLE_REQUIRED=YES")
    for index, (cycle, action, target) in enumerate(sequence, 1):
        print(f"R25_3_1_PLAN_ACTION_{index}=cycle:{cycle},action:{action},target:{target}")
    print("R25_3_1_PLAN_FINAL_STATE_RESTORED=YES")
    print("R25_3_1_NO_DEVICE_DRY_RUN=PASS")
    return 0
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.test_auto_actions and os.environ.get("R25_3_1_ALLOW_TEST_MODE") != "1":
    ...
    import r25_3_1_analyze as analyzer_module  # type: ignore
    ...
    required = [scripts / "r25_2_3_2_capture.py", scripts / "r25_2_3_2_hci_preflight.py", scripts / "r25_3_1_analyze.py"]
    ...
    print(f"R25_3_1_HCI_PREFLIGHT_STATUS={preflight.get('status')}")
    print("R25_3_1_HCI_CAPTURE_ALLOWED=YES")
    ...
    print("R25_3_1_STOCK_DEVELOPER_MODE_PAGE_ATTESTED=YES")
    ...
    print(f"R25_3_1_INITIAL_SEMANTIC_STATE={initial_state.upper()}")
    print("R25_3_1_ADB_TRANSPORT_DISAPPEARANCE_REQUIRED=NO")
    ...
    try:
        time.sleep(float(os.environ.get("R25_3_1_LOGCAT_STARTUP_SECONDS", "1")))
        ...
        print(f"R25_3_1_BASELINE_IDLE_SECONDS={args.baseline_seconds}")
        ...
        for cycle, action, target in action_sequence:
            ...
            operator_ready(
                f"\nACTION {action_id.upper()}\nCurrent semantic state: {current_state.upper()}\n"
                f"After Enter, perform exactly one stock {action.upper()} action. Complete the stock dialog if shown.\n",
                args.test_auto_actions,
            )
            before = semantic_sample(phone, glasses, args.adb, evidence, f"{action_id}-before")
            ...
            print(f"R25_3_1_{action_id.upper().replace('-', '_')}_SEMANTIC_TRANSITION=PASS")
            print(f"R25_3_1_{action_id.upper().replace('-', '_')}_CONTROL_CHANNEL_USABLE=YES")
    ...
    print("R25_3_1_POST_ACTION_BUGREPORT=PASS")

    metadata = {
        "schema": "rokid.r25.3.1.capture-metadata.v1", "release": RELEASE,
    ...
    print(f"R25_3_1_INITIAL_SEMANTIC_STATE={initial_state.upper()}")
    print(f"R25_3_1_FINAL_SEMANTIC_STATE={final_state.upper()}")
    print("R25_3_1_FINAL_STATE_RESTORED=YES")
    print("R25_3_1_ADB_TRANSPORT_DISAPPEARANCE_REQUIRED=NO")
    print("R25_3_1_CONTROL_CHANNEL_USABLE=YES")
    print("R25_3_1_CUSTOM_TRANSMISSION_ATTEMPTED=NO")
    print(f"R25_3_1_QUALIFICATION_OUTCOME={private['qualification_outcome']}")
    print(f"R1_3_3_2_25_3_1_ACCEPTANCE={private['acceptance']}")
    for suffix in ("-private-evidence.zip", "-private-analysis.zip", "-sanitized-publication.zip"):
        path = Path(str(out) + suffix)
        print(f"R25_3_1_ARTIFACT={path}")
        print(f"R25_3_1_ARTIFACT_SHA256={sha256_file(path)}")
    print(f"R25_3_1_OUTPUT={out}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("R25_3_1_CAPTURE_ABORTED=YES", file=sys.stderr)
        raise SystemExit(130)
    except CaptureFailure as exc:
        print(f"R25_3_1_CAPTURE_FAILURE={exc}", file=sys.stderr)
        raise SystemExit(1)
