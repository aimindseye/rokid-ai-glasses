#!/usr/bin/env python3
"""Offline r25.3.1 stock ADB-toggle semantic-oracle RFCOMM/HCI differential analyzer.

This analyzer never contacts a device. It consumes one capture directory produced
by r25_3_1_capture.py, reuses the accepted r25.2.3.2 btsnoop parser, attributes
...
UTC = dt.timezone.utc
RELEASE = "r1.3.3.2.25.3.1"
SCHEMA = "rokid.r25.3.1.stock-adb-toggle-semantic-differential.v1"
TARGET_DLCI = 6
...
def parse_hci_member(name: str, data: bytes, capture_start: dt.datetime, capture_end: dt.datetime) -> Dict[str, Any]:
    ...
    # Discover its L2CAP CIDs from the complete rolling snoop stream, then parse
    # only PDUs near the bounded capture interval.
    ...
    for pdu in window_pdus:
        ...
        rfcomm_parse_errors.extend(f"pdu_{pdu.source_record_index}:{error}" for error in errors)
    target_frames = sorted((frame for frame in frames if frame.dlci == TARGET_DLCI), key=lambda frame: frame.timestamp)
    ...
    # Whole-log reassembly diagnostics are retained, but only structural btsnoop
    # errors and RFCOMM parse failures in the bounded window block attribution.
    errors = list(parse_errors) + list(rfcomm_parse_errors)
...
def build_markdown(public: Dict[str, Any]) -> str:
    ...
    return f"""# {RELEASE} — stock ADB toggle semantic-oracle RFCOMM payload capture
...
- HCI UIH payload attribution: **{'PASS' if gates['hci_uih_payload_attributed'] else 'FAIL'}**
...
disappearance is not an acceptance condition. Raw UIH bytes remain private. Public artifacts contain directions, lengths, counts, hashes, and
...
def analyze(capture_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    if metadata.get("schema") != "rokid.r25.3.1.capture-metadata.v1":
    ...
    acceptance = (
        "PASS_STOCK_ADB_TOGGLE_SEMANTIC_ORACLE_UIH_DIFFERENTIAL_CAPTURE_AND_BOUNDED_FRAMING_RECOVERY"
        if pass_capture
        else "FAIL_STOCK_ADB_TOGGLE_SEMANTIC_ORACLE_UIH_DIFFERENTIAL_CAPTURE"
    )
    qualification = (
        "STOCK_ADB_SEMANTIC_ENABLE_DISABLE_UIH_PAYLOADS_ATTRIBUTED_DIFFERENTIAL_PROVEN_CUSTOM_REPLAY_NOT_ATTEMPTED"
        if pass_capture
        else "STOCK_ADB_TOGGLE_SEMANTIC_OR_UIH_DIFFERENTIAL_NOT_PROVEN"
    )
    private = {
        "schema": SCHEMA,
        "release": RELEASE,
    ...
    public = {
        "schema": "rokid.r25.3.1.public-status.v1",
        "release": RELEASE,
        "capture": {
    ...
    private_dir.mkdir()
    public_dir.mkdir()
    (private_dir / "r25.3.1-private-analysis.json").write_text(
        json.dumps(private, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (public_dir / "r25.3.1-runtime-status-summary.json").write_text(
        json.dumps(public, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (public_dir / "r25.3.1-stock-adb-toggle-semantic-oracle-rfcomm-payload-capture.md").write_text(
        build_markdown(public), encoding="utf-8"
    )
    (public_dir / "methodology.md").write_text(
        "# Methodology\n\nThe host records repeated stock enable/disable semantic transitions, using `persist.vendor.adb` and the stock UI switch while retaining a usable read-only ADB control channel, plus epoch logcat markers and a post-run bugreport. The offline analyzer reuses the accepted r25.2.3.2 btsnoop ACL/L2CAP/RFCOMM parser, attributes non-control UIH information bytes on DLCI 6 to non-overlapping action windows, subtracts exact idle-baseline signatures, and compares repeated enable and disable observations. Raw payload bytes remain private.\n",
        encoding="utf-8",
    )
    (public_dir / "limitations.md").write_text(
        "# Limitations\n\nA differential byte position is a bounded command-field candidate, not semantic proof. One phone, one glasses unit, one firmware/app build, and one paired account are in scope. Encryption, checksums, sequence fields, authorization, and reply semantics remain unresolved until independent captures and code correlation explain them. No captured payload is replayed.\n",
        encoding="utf-8",
    )
    print(f"R25_3_1_HCI_MEMBER_COUNT={len(hci_members)}")
    print("R25_3_1_HCI_UIH_ATTRIBUTION=" + ("PASS" if gates["hci_uih_payload_attributed"] else "FAIL"))
    print(f"R25_3_1_ENABLE_OBSERVATION_COUNT={counts['enable']}")
    print(f"R25_3_1_DISABLE_OBSERVATION_COUNT={counts['disable']}")
    print("R25_3_1_ENABLE_DISABLE_DIFFERENTIAL=" + ("PASS" if gates["enable_disable_differential_proven"] else "FAIL"))
    print(f"R25_3_1_APPLICATION_FRAMING_STATUS={differential['framing_candidate_status']}")
    print("R25_3_1_CUSTOM_TRANSMISSION_ATTEMPTED=NO")
    print(f"R25_3_1_QUALIFICATION_OUTCOME={qualification}")
    print(f"R1_3_3_2_25_3_1_ACCEPTANCE={acceptance}")
    return private
...
if __name__ == "__main__":
    try:
        ...
        print(f"R25_3_1_ANALYSIS_FAILURE={exc}", file=sys.stderr)
        raise SystemExit(1)
