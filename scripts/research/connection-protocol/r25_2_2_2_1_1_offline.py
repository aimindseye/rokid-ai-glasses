#!/usr/bin/env python3
"""Offline r25.2.2.2.1.1 RFCOMM timestamp/timebase and attempt-correlation repair analyzer.
...
SCHEMA_PRIVATE = "rokid.r25.2.2.2.1.1.private-analysis.v2"
SCHEMA_PUBLIC = "rokid.r25.2.2.2.1.1.public-full-closure.v2"
...
THREADTIME_RE = re.compile(r"(?<!\d)(?P<ts>\d\d-\d\d\s+\d\d:\d\d:\d\d\.\d{3,9})(?!\d)")
THREADTIME_PREFIX_RE = re.compile(
    r"^\s*(?P<ts>\d\d-\d\d\s+\d\d:\d\d:\d\d\.\d{3,9})"
    r"\s+(?P<pid>\d+)\s+(?P<tid>\d+)\s+(?P<priority>[VDIWEF])\s+(?P<body>.*)$"
)
EPOCH_PREFIX_RE = re.compile(
    r"^\s*(?P<ts>1\d{9}(?:\.\d{1,9})?)"
    r"\s+(?P<pid>\d+)\s+(?P<tid>\d+)\s+(?P<priority>[VDIWEF])\s+(?P<body>.*)$"
)
EPOCH_MS_PREFIX_RE = re.compile(
    r"^\s*(?P<ts>1\d{12})"
    r"\s+(?P<pid>\d+)\s+(?P<tid>\d+)\s+(?P<priority>[VDIWEF])\s+(?P<body>.*)$"
)
...
RFCOMM_CONTEXT_RE = re.compile(r"(?i)(rfcomm|bluetoothsocket|btsock|dlci|scn|port[_ -]?handle|bta[_ -]?jv)")
...
FIELD_PATTERNS = {
    ...
    "port_handle": [
        ...
        re.compile(r"(?i)\bBTA[_ -]?JvRfcommClose\b[^\n]{0,120}\bhandle\s*[=:]\s*((?:0x)?[0-9a-f]+)\b"),
        re.compile(r"(?i)\bon_(?:cli_)?rfc_close\b[^\n]{0,120}\bhandle\s*[=:]\s*((?:0x)?[0-9a-f]+)\b"),
    ],
    "scn": [
...
class Attempt:
    ...
    @staticmethod
    def _records_compatible(open_record: SourceLine, close_record: SourceLine) -> bool:
        open_handle = open_record.fields.get("port_handle")
        close_handle = close_record.fields.get("port_handle")
        if open_handle is not None and close_handle is not None and open_handle != close_handle:
            return False
        open_endpoint = open_record.fields.get("endpoint")
        close_endpoint = close_record.fields.get("endpoint")
        if open_endpoint and close_endpoint and not endpoint_values_compatible([open_endpoint, close_endpoint]):
            return False
        return bool(
            (open_handle is not None and close_handle is not None)
            or (open_endpoint and close_endpoint)
            or (open_handle is not None and close_handle is None)
            or (open_handle is None and close_handle is not None)
        )

    def lifecycle_pair(self) -> Optional[Tuple[int, int]]:
        for open_index in self.open_indices:
            for close_index in self.close_indices:
                if close_index <= open_index:
                    continue
                if self._records_compatible(self.records[open_index], self.records[close_index]):
                    return open_index, close_index
        return None

    def lifecycle_matched(self) -> bool:
        pair = self.lifecycle_pair()
        if pair is None:
            return False
        expected_handle = self.fields.get("port_handle")
        if expected_handle is None:
            return False
        open_record, close_record = (self.records[pair[0]], self.records[pair[1]])
        for record in (open_record, close_record):
            observed = record.fields.get("port_handle")
            if observed is not None and observed != expected_handle:
                return False
        return True
...
def parse_datetime(value: Any) -> Optional[dt.datetime]:
    ...
    if not text:
        return None
    if re.fullmatch(r"\d{10}(?:\.\d{1,9})?|\d{13}", text):
...
def normalized_log_body(line: str) -> str:
    for pattern in (EPOCH_PREFIX_RE, EPOCH_MS_PREFIX_RE, THREADTIME_PREFIX_RE):
        match = pattern.match(line)
        if match:
            body = match.group("body")
            return re.sub(r"\s+", " ", body.strip())
    body = re.sub(r"^.*?\b(?:[VDIWEF])/[^:]+:\s*", "", line.strip())
    return re.sub(r"\s+", " ", body)


def parse_threadtime_naive(text: str, year: int) -> Optional[dt.datetime]:
    try:
        return dt.datetime.strptime(f"{year}-{text}", "%Y-%m-%d %H:%M:%S.%f")
    except ValueError:
        return None


def parse_epoch_prefix(line: str) -> Tuple[Optional[dt.datetime], Optional[str]]:
    match = EPOCH_PREFIX_RE.match(line)
    if match:
        parsed = parse_datetime(float(match.group("ts")))
        return parsed, match.group("ts")
    match = EPOCH_MS_PREFIX_RE.match(line)
    if match:
        parsed = parse_datetime(int(match.group("ts")))
        return parsed, match.group("ts")
    return None, None
...
def metadata_timebase_candidates(path: Path, midpoint: dt.datetime) -> List[TimebaseResolution]:
    numeric_keys = {
        "threadtime_utc_offset_minutes", "phone_utc_offset_minutes",
        "device_utc_offset_minutes", "timezone_offset_minutes", "utc_offset_minutes",
    }
    zone_keys = {
        "phone_timezone", "device_timezone", "timezone_name", "time_zone",
        "timezone", "persist.sys.timezone",
    }
    candidates: List[TimebaseResolution] = []
    for source, raw in iter_archive_members(path):
        basename = Path(source.split("!", 1)[0]).name.lower()
        if not any(token in basename for token in ("metadata", "run-info", "handoff", "getprop", "bugreport")):
            continue
        text = safe_text(raw)
        if text is None:
            continue
                ...
                for key_path, value in walk_json(obj):
                    ...
                    if key in numeric_keys:
                        ...
                        if -14 * 60 <= offset <= 14 * 60:
                            candidates.append(TimebaseResolution(offset, "metadata_numeric_offset", "explicit", (f"{source}:{key_path}",)))
                    if key in zone_keys and isinstance(value, str):
                        offset = timezone_offset_minutes(value.strip(), midpoint)
                        if offset is not None:
                            candidates.append(TimebaseResolution(offset, "metadata_timezone", "explicit", (f"{source}:{key_path}",), value.strip()))
        for line_number, line in enumerate(text.splitlines(), 1):
            zone_match = re.search(
                r"(?:\[?persist\.sys\.timezone\]?\s*[:=]\s*\[?|\bTimezone\s*[:=]\s*)([A-Za-z_]+/[A-Za-z0-9_+\-]+)",
                line,
                re.IGNORECASE,
            )
            if zone_match:
                zone_name = zone_match.group(1).rstrip("]")
                offset = timezone_offset_minutes(zone_name, midpoint)
                if offset is not None:
                    candidates.append(TimebaseResolution(offset, "bugreport_timezone_property", "explicit", (f"{source}:{line_number}",), zone_name))
            offset_match = re.search(
                r"\b(?:threadtime_utc_offset_minutes|phone_utc_offset_minutes|device_utc_offset_minutes|timezone_offset_minutes|utc_offset_minutes)\s*[:=]\s*([+-]?\d{1,4})\b",
                line,
                re.IGNORECASE,
            )
            if offset_match:
                ...
                if -14 * 60 <= offset <= 14 * 60:
                    candidates.append(TimebaseResolution(offset, "metadata_text_offset", "explicit", (f"{source}:{line_number}",)))
    return candidates


def paired_log_timebase(path: Path, year: int) -> Optional[TimebaseResolution]:
    epoch_by_body: Dict[str, List[Tuple[dt.datetime, str]]] = defaultdict(list)
    thread_by_body: Dict[str, List[Tuple[dt.datetime, str]]] = defaultdict(list)
    for source, raw in iter_archive_members(path):
        text = safe_text(raw)
        if text is None:
            continue
        for line_number, line in enumerate(text.splitlines(), 1):
            body = normalized_log_body(line)
            if len(body) < 24 or (not extract_fields(line) and not classify_flags(line)):
                continue
            epoch, _ = parse_epoch_prefix(line)
            if epoch is not None:
                epoch_by_body[body].append((epoch, f"{source}:{line_number}"))
                continue
            match = THREADTIME_PREFIX_RE.match(line)
            if match:
                naive = parse_threadtime_naive(match.group("ts"), year)
                if naive is not None:
                    thread_by_body[body].append((naive, f"{source}:{line_number}"))
    votes: Counter[int] = Counter()
    sources: Dict[int, List[str]] = defaultdict(list)
    unique_bodies: Dict[int, set[str]] = defaultdict(set)
    for body in sorted(set(epoch_by_body) & set(thread_by_body)):
        best: Optional[Tuple[float, int, str, str]] = None
        for epoch, epoch_source in epoch_by_body[body][:4]:
            epoch_naive = epoch.astimezone(dt.timezone.utc).replace(tzinfo=None)
            for local_naive, thread_source in thread_by_body[body][:4]:
                raw_minutes = (local_naive - epoch_naive).total_seconds() / 60.0
                rounded = int(round(raw_minutes / 15.0) * 15)
                residual = abs(raw_minutes - rounded) * 60.0
                if not (-14 * 60 <= rounded <= 14 * 60) or residual > 2.0:
                    continue
                candidate = (residual, rounded, epoch_source, thread_source)
                if best is None or candidate < best:
                    best = candidate
        if best is not None:
            _, offset, epoch_source, thread_source = best
            votes[offset] += 1
            unique_bodies[offset].add(body)
            if len(sources[offset]) < 8:
                sources[offset].extend([epoch_source, thread_source])
    if not votes:
        return None
    ordered = votes.most_common()
    best_offset, best_count = ordered[0]
    if best_count < 2 or (len(ordered) > 1 and ordered[1][1] == best_count):
        return None
    return TimebaseResolution(
        best_offset,
        "paired_epoch_threadtime_records",
        "explicit_cross_format_correlation",
        tuple(dict.fromkeys(sources[best_offset])),
        matched_pair_count=len(unique_bodies[best_offset]),
    )


def has_semantic_threadtime(path: Path) -> bool:
    for _, raw in iter_archive_members(path):
        text = safe_text(raw)
        if text is None:
            continue
        for line in text.splitlines():
            if THREADTIME_PREFIX_RE.match(line) and (extract_fields(line) or classify_flags(line)):
                return True
    return False


def resolve_timebase(path: Path, start: dt.datetime, end: dt.datetime) -> TimebaseResolution:
    midpoint = start + (end - start) / 2
    explicit = metadata_timebase_candidates(path, midpoint)
    paired = paired_log_timebase(path, start.year)
    candidates = list(explicit)
    if paired is not None:
        candidates.append(paired)
    if candidates:
        offsets = {candidate.offset_minutes for candidate in candidates}
        if len(offsets) != 1:
            detail = ", ".join(f"{item.method}={item.offset_minutes}" for item in candidates)
            raise AnalysisError(f"Conflicting explicit threadtime timebases: {detail}")
        preferred = sorted(
            candidates,
            key=lambda item: (
                0 if item.method.startswith("metadata") or item.method.startswith("bugreport") else 1,
                -item.matched_pair_count,
                item.method,
            ),
        )[0]
        merged_sources = tuple(dict.fromkeys(source for item in candidates for source in item.sources))
        methods = "+".join(sorted({item.method for item in candidates}))
        confidence = "explicit_correlated" if len(candidates) > 1 else preferred.confidence
        zone_name = next((item.timezone_name for item in candidates if item.timezone_name), None)
        pair_count = max((item.matched_pair_count for item in candidates), default=0)
        return TimebaseResolution(preferred.offset_minutes, methods, confidence, merged_sources, zone_name, pair_count)
    if has_semantic_threadtime(path):
        raise AnalysisError(
            "Semantic Android threadtime records are present, but no explicit metadata, timezone property, "
            "or paired epoch/threadtime provenance could establish their UTC offset"
        )
    return TimebaseResolution(None, "threadtime_not_required", "not_applicable")


def parse_line_timestamp(
    line: str,
    year: int,
    timebase: TimebaseResolution,
) -> Tuple[Optional[dt.datetime], Optional[str], Optional[str], Optional[str]]:
    match = ISO_RE.search(line)
    if match:
        parsed = parse_datetime(match.group("ts"))
        return parsed, match.group("ts"), "iso8601", "embedded_utc_or_offset"
    parsed_epoch, epoch_text = parse_epoch_prefix(line)
    if parsed_epoch is not None:
        return parsed_epoch, epoch_text, "unix_epoch", "epoch_is_utc"
    ...
    if match:
        ...
        naive = parse_threadtime_naive(match.group("ts"), year)
        if naive is not None:
            local_as_utc = naive.replace(tzinfo=dt.timezone.utc)
            parsed = local_as_utc - dt.timedelta(minutes=timebase.offset_minutes)
            return parsed, match.group("ts"), "android_threadtime", timebase.method
        ...
        if isinstance(data, dict):
            for key in ("timestamp_utc", "ts_utc", "time_utc", "timestamp", "ts", "time", "epoch_ms", "timestamp_ms"):
                if key in data:
                    ...
                    if parsed:
                        return parsed, str(data[key]), "json_timestamp", f"json:{key}"
    return None, None, None, None
...
def canonical_fingerprint(timestamp: dt.datetime, flags: Sequence[str], fields: Dict[str, Any], line: str) -> str:
    semantic = {
        ...
        "flags": sorted(flags),
        "fields": {key: fields[key] for key in sorted(fields) if key != "source_pid"},
        "body": normalized_log_body(line),
    }
...
def source_class(source: str) -> str:
    ...
    if "bugreport" in lowered:
        return "bugreport"
    if "logcat" in lowered:
        return "logcat"
    if "metadata" in lowered or "run-info" in lowered:
        return "metadata"
    return "other"


def collect_records(path: Path, start: dt.datetime, end: dt.datetime) -> Tuple[List[SourceLine], Dict[str, Any]]:
    ...
    stats: Dict[str, Any] = {
        ...
        "timestamp_kind_counts": {},
        "source_class_counts": {},
        "timebase_provenance": timebase.as_dict(),
        "threadtime_utc_offset_minutes": timebase.offset_minutes,
    }
    timestamp_kinds: Counter[str] = Counter()
    ...
    for source, raw in iter_archive_members(path):
        ...
        if text is None:
            continue
        for line_number, line in enumerate(text.splitlines(), 1):
            timestamp, timestamp_text, timestamp_kind, timebase_source = parse_line_timestamp(line, start.year, timebase)
            if timestamp is None:
                continue
            stats["raw_timestamped_lines"] += 1
            timestamp_kinds[str(timestamp_kind)] += 1
            source_classes[source_class(source)] += 1
            ...
            record = SourceLine(
                ...
                timestamp_kind=timestamp_kind or "unknown",
                timebase_source=timebase_source or "unknown",
                event_flags=flags,
                fields=fields,
    ...
    stats["timestamp_kind_counts"] = dict(sorted(timestamp_kinds.items()))
    stats["source_class_counts"] = dict(sorted(source_classes.items()))
    return records, stats

def record_compatible_with_attempt(record: SourceLine, attempt: Attempt) -> bool:
    seed_endpoints = attempt.endpoint_values or ([str(attempt.fields["endpoint"])] if "endpoint" in attempt.fields else [])
    candidate_endpoint = record.fields.get("endpoint")
    if candidate_endpoint and seed_endpoints and not endpoint_values_compatible([*seed_endpoints, candidate_endpoint]):
        return False
    seed_handle = attempt.fields.get("port_handle")
    candidate_handle = record.fields.get("port_handle")
    if seed_handle is not None and candidate_handle is not None and seed_handle != candidate_handle:
        return False
    return True


def attempt_window(attempt: Attempt, next_anchor: Optional[dt.datetime]) -> Tuple[dt.datetime, dt.datetime]:
    starts = [record.timestamp for record in attempt.records if "attempt_start" in record.event_flags]
    ends = [record.timestamp for record in attempt.records if "attempt_end" in record.event_flags]
    clients = [record.timestamp for record in attempt.records if "client_connect" in record.event_flags]
    anchor = min(clients or starts or [record.timestamp for record in attempt.records])
    window_start = min(starts) if starts else anchor - dt.timedelta(seconds=10)
    window_end = max(ends) if ends else anchor + dt.timedelta(seconds=60)
    if next_anchor is not None:
        window_end = min(window_end, next_anchor - dt.timedelta(microseconds=1))
    return window_start, window_end


def absorb_native_records(attempt: Attempt, records: Sequence[SourceLine], assigned: set[int], next_anchor: Optional[dt.datetime]) -> None:
    attempt.aggregate()
    window_start, window_end = attempt_window(attempt, next_anchor)
    candidates = [
        record for record in records
        if id(record) not in assigned
        and record.attempt_id is None
        and window_start <= record.timestamp <= window_end
        and record_compatible_with_attempt(record, attempt)
    ]
    open_seen = any("open" in record.event_flags for record in attempt.records)
    for record in candidates:
        ...
        if open_seen and "close" in record.event_flags:
            # Include same-timestamp duplicates/adjacent semantic records, then stop.
            close_time = record.timestamp
            for trailing in candidates:
                if id(trailing) in assigned:
                    continue
                if trailing.timestamp <= close_time + dt.timedelta(milliseconds=2):
                    attempt.records.append(trailing)
                    assigned.add(id(trailing))
            break
...
def group_attempts(records: Sequence[SourceLine]) -> List[Attempt]:
    ...
    for attempt in explicit_attempts:
        clients = [record.timestamp for record in attempt.records if "client_connect" in record.event_flags]
        anchors.append(min(clients or [record.timestamp for record in attempt.records]))
    for index, attempt in enumerate(explicit_attempts):
        next_anchor = anchors[index + 1] if index + 1 < len(anchors) else None
        absorb_native_records(attempt, records, assigned, next_anchor)

    attempts: List[Attempt] = list(explicit_attempts)
    unassigned_clients = [
        record for record in records
        if id(record) not in assigned and "client_connect" in record.event_flags
    ]
    for ordinal, client in enumerate(unassigned_clients, 1):
        if id(client) in assigned:
            continue
        next_anchor = next(
            (other.timestamp for other in unassigned_clients if other.timestamp > client.timestamp and id(other) not in assigned),
            None,
        )
        attempt = Attempt(f"inferred-{ordinal}", [client])
        assigned.add(id(client))
        absorb_native_records(attempt, records, assigned, next_anchor)
...
def evidence_ref(record: SourceLine, include_text: bool) -> Dict[str, Any]:
    result: Dict[str, Any] = {
        ...
        "timebase_source": record.timebase_source,
...
def attempt_private(attempt: Attempt) -> Dict[str, Any]:
    return {
        ...
        "lifecycle_pair_indices": list(attempt.lifecycle_pair()) if attempt.lifecycle_pair() else None,
...
def render_markdown(public: Dict[str, Any]) -> str:
    ...
    return f"""# r1.3.3.2.25.2.2.2.1.1 — Android Bugreport Epoch/Timebase RFCOMM Closure Repair
...
def run_analysis(source_zip: Path, output: Path, expected_source_sha256: Optional[str]) -> Dict[str, Any]:
    ...
    private = {
        ...
        "timebase_provenance": stats["timebase_provenance"],
    ...
    write_json(analysis_dir / "r25.2.2.2.1.1-private-analysis.json", private)
    ...
    public = {
        ...
        "evidence_controls": {
            ...
            "offline_only": True,
            "unix_epoch_prefix_parsed_as_utc": True,
            "threadtime_requires_explicit_provenance": True,
            "semantic_offset_maximization_forbidden": True,
        },
        "timebase_provenance": stats["timebase_provenance"],
        "selection_stats": stats,
        "attempt_count": len(attempts),
    ...
    write_json(publication_dir / "r25.2.2.2.1.1-full-zero-payload-closure.json", public)
    write_text(publication_dir / "r25.2.2.2.1.1-full-zero-payload-closure.md", render_markdown(public))
    status = {
        "schema": SCHEMA_STATUS,
        "release": "r1.3.3.2.25.2.2.2.1.1",
        ...
        "timebase_method": stats["timebase_provenance"]["method"],
        "threadtime_utc_offset_minutes": stats["timebase_provenance"]["threadtime_utc_offset_minutes"],
        "semantic_offset_maximization_used": False,
    }
    write_json(publication_dir / "runtime-status-summary.json", status)
    write_text(publication_dir / "methodology.md", """# Methodology

1. Verify the locked source private-archive SHA-256.
2. Read the exact UTC start/end interval from archive metadata.
3. Parse Android bugreport `epoch_seconds PID TID priority` records directly as UTC.
4. Resolve Android threadtime only from explicit metadata/timezone properties or
   paired epoch/threadtime copies of the same log records; semantic-count
   maximization is prohibited.
5. Admit only timestamped records inside the closed metadata interval.
6. Deduplicate identical overlapping logcat and bugreport events by UTC timestamp,
   normalized message body, fields, and flags.
7. Correlate explicit app attempt records with compatible native records inside one
   non-overlapping bounded lifecycle window.
8. Require client semantics, UUID, endpoint, UID, PID, slot 45, handle 29, SCN 3,
   DLCI 6, MTU 990, a compatible ordered open/close pair, and zero payload.
9. Fail closed on unresolved/conflicting timebases, cross-attempt merging, identity
   conflicts, missing lifecycle evidence, nonzero payload, or multiple qualifiers.
""")
    write_text(publication_dir / "limitations.md", """# Limitations

This repair addresses timestamp parsing, timebase provenance, deduplication, and
same-attempt correlation only. It does not claim application payload semantics,
bidirectional transfer, stock Hi Rokid equivalence beyond the observed tuple, or
behavior outside the metadata interval. A PASS remains dependent on the contents
of the locked real archive. Raw endpoint and runtime UUID values remain private
and are replaced by SHA-256-derived tokens publicly.
""")
    ...
    print(f"R25_2_2_2_1_1_SOURCE_PRIVATE_ZIP_SHA256={source_sha}")
    print(f"R25_2_2_2_1_1_METADATA_INTERVAL_START={start.isoformat().replace('+00:00', 'Z')}")
    print(f"R25_2_2_2_1_1_METADATA_INTERVAL_END={end.isoformat().replace('+00:00', 'Z')}")
    provenance = stats["timebase_provenance"]
    print(f"R25_2_2_2_1_1_TIMEBASE_METHOD={provenance['method']}")
    print(f"R25_2_2_2_1_1_THREADTIME_UTC_OFFSET_MINUTES={provenance['threadtime_utc_offset_minutes']}")
    print(f"R25_2_2_2_1_1_TIMEBASE_PAIR_COUNT={provenance['matched_pair_count']}")
    print("R25_2_2_2_1_1_SEMANTIC_OFFSET_MAXIMIZATION=NO")
    print(f"R25_2_2_2_1_1_INTERVAL_FILTER=PASS")
    print(f"R25_2_2_2_1_1_DEDUPLICATION=PASS")
    print(f"R25_2_2_2_1_1_ATTEMPT_COUNT={len(attempts)}")
    print(f"R25_2_2_2_1_1_ACCEPTED_ATTEMPT_COUNT={len(accepted_attempts)}")
    if accepted_public:
        fields = accepted_public["fields"]
        print("R25_2_2_2_1_1_ANDROID_CLIENT_SEMANTIC=YES")
        print(f"R25_2_2_2_1_1_SLOT={fields.get('slot')}")
        print(f"R25_2_2_2_1_1_PORT_HANDLE={fields.get('port_handle')}")
        print(f"R25_2_2_2_1_1_SCN={fields.get('scn')}")
        print(f"R25_2_2_2_1_1_DLCI={fields.get('dlci')}")
        print(f"R25_2_2_2_1_1_MTU={fields.get('mtu')}")
        print("R25_2_2_2_1_1_MATCHING_OPEN_CLOSE=YES")
        print("R25_2_2_2_1_1_ZERO_PAYLOAD=YES")
    print(f"R25_2_2_2_1_1_QUALIFICATION_OUTCOME={public['qualification_outcome']}")
    print(f"R1_3_3_2_25_2_2_2_1_1_ACCEPTANCE={public['acceptance']}")
    print(f"R25_2_2_2_1_1_OUTPUT={output}")
    return public


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Offline r25.2.2.2.1.1 epoch/timebase attempt-correlation repair")
    ...
    try:
        ...
        print(f"R1_3_3_2_25_2_2_2_1_1_ACCEPTANCE=FAIL", file=sys.stderr)
