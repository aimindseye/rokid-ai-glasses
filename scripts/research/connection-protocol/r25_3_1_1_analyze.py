#!/usr/bin/env python3
"""Offline r25.3.1.1 stock ADB-toggle semantic-oracle RFCOMM/HCI differential analyzer.
...
SCHEMA = "rokid.r25.3.1.1.stock-adb-toggle-semantic-differential.v1"
TARGET_DLCI = 6
...
def analyze(capture_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    public = {
        "schema": "rokid.r25.3.1.1.public-status.v1",
        "release": RELEASE,
        "capture": {
    ...
    private_dir.mkdir()
    public_dir.mkdir()
    (private_dir / "r25.3.1.1-private-analysis.json").write_text(
        json.dumps(private, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (public_dir / "r25.3.1.1-runtime-status-summary.json").write_text(
        json.dumps(public, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (public_dir / "r25.3.1.1-stock-adb-toggle-semantic-oracle-rfcomm-payload-capture.md").write_text(
    ...
    print(f"R25_3_1_1_HCI_MEMBER_COUNT={len(hci_members)}")
    print("R25_3_1_1_HCI_UIH_ATTRIBUTION=" + ("PASS" if gates["hci_uih_payload_attributed"] else "FAIL"))
    print(f"R25_3_1_1_ENABLE_OBSERVATION_COUNT={counts['enable']}")
    print(f"R25_3_1_1_DISABLE_OBSERVATION_COUNT={counts['disable']}")
    print("R25_3_1_1_ENABLE_DISABLE_DIFFERENTIAL=" + ("PASS" if gates["enable_disable_differential_proven"] else "FAIL"))
    print(f"R25_3_1_1_APPLICATION_FRAMING_STATUS={differential['framing_candidate_status']}")
    print("R25_3_1_1_CUSTOM_TRANSMISSION_ATTEMPTED=NO")
    print(f"R25_3_1_1_QUALIFICATION_OUTCOME={qualification}")
    print(f"R1_3_3_2_25_3_1_1_ACCEPTANCE={acceptance}")
    return private
...
if __name__ == "__main__":
    try:
        ...
        print(f"R25_3_1_1_ANALYSIS_FAILURE={exc}", file=sys.stderr)
        raise SystemExit(1)
