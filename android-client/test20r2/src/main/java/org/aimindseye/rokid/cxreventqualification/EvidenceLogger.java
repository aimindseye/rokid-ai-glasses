...
final class EvidenceLogger {
    private static final String TAG = "Test20R2";
    ...
    EvidenceLogger(Context context, String runId, String firmwareLabel) throws IOException {
        ...
        File root = new File(context.getExternalFilesDir(null), "test20-r2");
        if (!root.exists() && !root.mkdirs()) {
            throw new IOException("Cannot create evidence directory: " + root);
        }
        outputFile = new File(root, "test20-r2-" + this.runId + ".jsonl");
    }
    ...
    synchronized void event(String eventType, JSONObject details) {
        ...
        try {
            record.put("schema", Test20R2Contract.EVENT_SCHEMA);
    ...
    static JSONObject details(Object... values) {
        ...
        for (int index = 0; index + 1 < values.length; index += 2) {
            try {
                ...
                // The caller still receives every serializable scalar.
            }
        }
        return object;
    }
    ...
    private static String sanitize(String value) {
        String normalized =
                value == null ? "run" : value.replaceAll("[^A-Za-z0-9._-]", "_");
