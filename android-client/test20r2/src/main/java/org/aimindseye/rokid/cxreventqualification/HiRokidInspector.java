...
final class HiRokidInspector {
    ...
    JSONObject inspect() {
        ...
        Intent authIntent =
                new Intent(Test20R2Contract.AUTH_ACTION).setPackage(packageName);
        ResolveInfo auth =
        ...
        Intent serviceIntent =
                new Intent(Test20R2Contract.MEDIA_SERVICE_ACTION).setPackage(packageName);
    ...
    private String installedPackage(PackageManager manager) {
        if (isInstalled(manager, Test20R2Contract.GLOBAL_HI_ROKID_PACKAGE)) {
            return Test20R2Contract.GLOBAL_HI_ROKID_PACKAGE;
        }
        if (isInstalled(manager, Test20R2Contract.CHINA_HI_ROKID_PACKAGE)) {
            return Test20R2Contract.CHINA_HI_ROKID_PACKAGE;
        }
        return null;
    }
