...
final class AuthorizationController {
    ...
    void request() {
        ...
        logger.event("authorization_requested", EvidenceLogger.details(
                "request_code", Test20R32Contract.AUTH_REQUEST_CODE,
                "token_value_logged", false
        ));

        Intent explicit = new Intent().setComponent(new ComponentName(
                Test20R32Contract.GLOBAL_HI_ROKID_PACKAGE,
                Test20R32Contract.AUTH_ACTIVITY_CLASS
        ));
        try {
            activity.startActivityForResult(explicit, Test20R32Contract.AUTH_REQUEST_CODE);
        ...
        Intent fallback = new Intent(Test20R32Contract.AUTH_ACTION)
                .setPackage(Test20R32Contract.GLOBAL_HI_ROKID_PACKAGE);
        try {
            activity.startActivityForResult(fallback, Test20R32Contract.AUTH_REQUEST_CODE);
            logger.event("authorization_activity_started",
    ...
    void handleResult(int requestCode, int resultCode, Intent data) {
        if (requestCode != Test20R32Contract.AUTH_REQUEST_CODE) {
            return;
        }
    ...
    private static String directToken(Intent data) {
        ...
        String token = data.getStringExtra(Test20R32Contract.AUTH_TOKEN_EXTRA);
