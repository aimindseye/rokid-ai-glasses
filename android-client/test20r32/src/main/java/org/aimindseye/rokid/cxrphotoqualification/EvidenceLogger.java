...
final class EvidenceLogger {
    private static final String TAG = "Test20R32";
    ...
    EvidenceLogger(Context context, String runId, String firmwareLabel) throws IOException {
        ...
        File root = new File(context.getExternalFilesDir(null), "test20-r3-2");
        if (!root.exists() && !root.mkdirs()) {
            throw new IOException("Cannot create evidence directory: " + root);
        }
        outputFile = new File(root, "test20-r3-2-" + this.runId + ".jsonl");
    }
    ...
    synchronized void event(String eventType, JSONObject details) {
        ...
        try {
            record.put("schema", Test20R32Contract.EVENT_SCHEMA);
    ...
    static String sha256(byte[] value) {
        try {
            ...
            byte[] output = digest.digest(value == null ? new byte[0] : value);
