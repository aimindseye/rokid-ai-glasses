...
final class AuthorizationController {
    ...
    void request() {
        if (requested) {
            logger.event("authorization_rejected", EvidenceLogger.details("reason", "already_requested"));
        ...
        logger.event("authorization_requested", EvidenceLogger.details(
                "request_code", Test19R2Contract.AUTH_REQUEST_CODE,
                "token_value_logged", false
        ));

        Intent explicit = new Intent().setComponent(new ComponentName(
                Test19R2Contract.GLOBAL_HI_ROKID_PACKAGE,
                Test19R2Contract.AUTH_ACTIVITY_CLASS
        ));
        try {
            activity.startActivityForResult(explicit, Test19R2Contract.AUTH_REQUEST_CODE);
            logger.event("authorization_activity_started", EvidenceLogger.details(
                    "mode", "explicit_global_component"
            ));
            return;
        ...
        Intent fallback = new Intent(Test19R2Contract.AUTH_ACTION)
                .setPackage(Test19R2Contract.GLOBAL_HI_ROKID_PACKAGE);
        try {
            activity.startActivityForResult(fallback, Test19R2Contract.AUTH_REQUEST_CODE);
            logger.event("authorization_activity_started", EvidenceLogger.details(
                    "mode", "action_global_package"
            ));
    ...
    void handleResult(int requestCode, int resultCode, Intent data) {
        if (requestCode != Test19R2Contract.AUTH_REQUEST_CODE) {
            return;
        }
    ...
    private static String directToken(Intent data) {
        ...
        String token = data.getStringExtra(Test19R2Contract.AUTH_TOKEN_EXTRA);
    ...
    private static ReflectionResult reflectedToken(int resultCode, Intent data) {
        try {
            ...
            try {
                ...
                // Static Java helper variants do not expose INSTANCE.
            }
            ...
            return new ReflectionResult(token, "authorization_helper_reflection", result.getClass().getName());
        } catch (Exception error) {
            return new ReflectionResult(null, "reflection_failed_" + error.getClass().getSimpleName(), "");
        }
    }
