...
final class EvidenceLogger {
    private static final String TAG = "Test19R2";
    ...
    EvidenceLogger(Context context, String runId, String firmwareLabel) throws IOException {
        ...
        File root = new File(context.getExternalFilesDir(null), "test19-r2");
        if (!root.exists() && !root.mkdirs()) {
            throw new IOException("Cannot create evidence directory: " + root);
        }
        outputFile = new File(root, "test19-r2-" + this.runId + ".jsonl");
    }
    ...
    synchronized void event(String eventType, JSONObject details) {
        ...
        try {
            record.put("schema", Test19R2Contract.EVENT_SCHEMA);
    ...
    static JSONObject details(Object... values) {
        ...
        for (int index = 0; index + 1 < values.length; index += 2) {
            try {
                ...
                // The caller still receives all serializable fields.
            }
        }
        return object;
    }
