...
final class HiRokidInspector {
    ...
    JSONObject inspect() {
        ...
        Intent authIntent = new Intent(Test19R2Contract.AUTH_ACTION).setPackage(packageName);
        ResolveInfo auth = manager.resolveActivity(authIntent, PackageManager.MATCH_DEFAULT_ONLY);
        ActivityInfo authInfo = auth == null ? null : auth.activityInfo;
        detailsSafe(details, "authorization_resolved", authInfo != null);
        detailsSafe(details, "authorization_exported", authInfo != null && authInfo.exported);
        detailsSafe(details, "authorization_component",
                authInfo == null ? "" : new ComponentName(authInfo.packageName, authInfo.name).flattenToShortString());

        Intent serviceIntent = new Intent(Test19R2Contract.MEDIA_SERVICE_ACTION).setPackage(packageName);
        ResolveInfo service = manager.resolveService(serviceIntent, 0);
        ServiceInfo serviceInfo = service == null ? null : service.serviceInfo;
        detailsSafe(details, "media_service_resolved", serviceInfo != null);
        detailsSafe(details, "media_service_exported", serviceInfo != null && serviceInfo.exported);
        detailsSafe(details, "media_service_component",
                serviceInfo == null ? "" : new ComponentName(serviceInfo.packageName, serviceInfo.name).flattenToShortString());
        return details;
    }

    String installedPackage(PackageManager manager) {
        if (isInstalled(manager, Test19R2Contract.GLOBAL_HI_ROKID_PACKAGE)) {
            return Test19R2Contract.GLOBAL_HI_ROKID_PACKAGE;
        }
        if (isInstalled(manager, Test19R2Contract.CHINA_HI_ROKID_PACKAGE)) {
            return Test19R2Contract.CHINA_HI_ROKID_PACKAGE;
        }
        return null;
    }
